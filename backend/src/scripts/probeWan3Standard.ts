/**
 * Sonda manual, descartável — RODADA 22, 31/08/2026.
 *
 * UMA chamada real a `alibaba/wan-3.0/reference-to-video` (o PADRÃO, não o
 * Prime da RODADA 20) — mesma técnica de fora-do-pipeline (endpoint fora
 * de `ENDPOINT_CATALOG`, fala com a fila da fal DIRETO, sem `falSubmit`).
 *
 * Parâmetros IDÊNTICOS à RODADA 20 de propósito — mesmo prompt, mesmas
 * duas imagens de referência (reaproveitadas, sem novo upload), mesmo
 * seed — para isolar o MODELO como única variável na comparação.
 *
 * Preço MEDIDO na documentação oficial (RODADA 21): US$0,05/s a 480p —
 * 24s = US$1,20.
 *
 *   docker compose exec backend npx tsx src/scripts/probeWan3Standard.ts
 */
...
// Reaproveitadas da RODADA 20 — ainda acessíveis, sem novo upload.
const FOTO_URL = "https://v3b.fal.media/files/b/0aa88e12/JSCNwj_bzbZKzp98XMiSq_asset.png";
const COMPOSICAO_2_URL = "https://v3b.fal.media/files/b/0aa87c2a/invwgbw2J0_v3jahS3Y1j_jPD6Kvvf.png";
const ENDPOINT = "alibaba/wan-3.0/reference-to-video"; // PADRÃO — sem "-prime"
const QUEUE_BASE = "https://queue.fal.run";
const SEED_V20 = 1687738215;

// MESMO texto da RODADA 20, palavra por palavra — não "melhorado".
const PROMPT =
...
async function main() {
  ...
  const corpo = {
    prompt: PROMPT,
    reference_image_urls: [FOTO_URL, COMPOSICAO_2_URL],
    duration: 24,
    resolution: "480p",
    aspect_ratio: "16:9",
    audio: false,
  ...
  console.log("Image 1 (rosto):", FOTO_URL);
  console.log("Image 2 (cena/traje):", COMPOSICAO_2_URL);
  console.log("seed solicitado:", SEED_V20);
  ...
  const seedRecebido = resultData?.seed;
  console.log("\nseed solicitado:", SEED_V20, "| seed recebido:", seedRecebido, "| igual?", seedRecebido === SEED_V20);
  ...
  await writeFile("/tmp/v22-wan3-standard-resultado.mp4", buf);
  console.log("\nvídeo salvo em /tmp/v22-wan3-standard-resultado.mp4 (", buf.length, "bytes )");
  console.log("duração reportada:", resultData?.duration);
  console.log("custo real esperado: 24 x US$0,05 = US$1,20 (480p)");
}
